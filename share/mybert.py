...
def get_max_mask(arr,K=1):
  ...
  print('\n\n\n',
      ...
      ' VALUES', np.shape(values),
        'temp_indices', np.shape(temp_indices),
        'full_indices', np.shape(full_indices),
            '\n\n\n')
...
class Encoder(tf.keras.layers.Layer):
  ...
  def call_z(self, x, training, mask,z):
  ...
  def call_z2(self, x, training, mask,z):
    ...
    print('x', np.shape(x), x.dtype,
          'z', np.shape(z), z.dtype)
...
class myBert(tf.keras.Model):
  ...
  def call(self, inp, training, enc_padding_mask, from_logits=False,temper=1e-5):
  ...
  def call_enc(self,inp,training,enc_padding_mask):
    enc_output = self.encoder(inp, training, enc_padding_mask)  # (batch_size, inp_seq_len, d_model)
    return(enc_output)
  def call_z(self,inp,training,enc_padding_mask,z):
    enc_output = self.encoder.call_z(inp,training,enc_padding_mask,z)
    return(enc_output)
